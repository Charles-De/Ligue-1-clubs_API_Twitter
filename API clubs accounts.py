...
def add_to_csv(header, list, granularity, outfile_name):   #header is the name of the clubs, list is the list of count per time slot
    list_times_existing = []
    exists = False
    #read
    if os.path.exists(outfile_name + "_"+ granularity + ".csv"):    #checks if the file already exists or not
        exists = True
        csv_file = open(outfile_name + "_"+ granularity + ".csv", "r", encoding='utf-8', newline='')
        csv_existing = csv.reader(csv_file)
        for row in csv_existing:
            list_times_existing.append(row[0])
        csv_file.close()
    #write
    csv_file = open(outfile_name + "_"+ granularity + ".csv", "a", encoding='utf-8', newline='')
...
def count_ligue1(file, granularity, outfile_name): #granularity can be 'day', 'hour', 'minute'
        ...
        if (club_line != "" or club_line != "\n"):
            ...
            if zero_is_first == 0:
                ...
                for time in result["data"]:
                    list_per_time[index].append(time["tweet_count"])
                    index += 1
    add_to_csv(header_list, list_per_time, granularity, outfile_name)
...
def main():
    ...
    count_ligue1("list_clubs_compte", 'hour', "result_number_accounts")   #second argument is 'day', 'hour' or'minute'
